...
VALUES ('NVHCM0010', N'Nguyễn Xuân Thể', '056201036758', '2003-08-10', N'Nam', N'71 Tân Lập 1, Hiệp Phú, Thủ Đức, Hồ Chí Minh', '0706048103', N'Quản lý', 'CNHCM', '')
GO
INSERT INTO NHANVIEN (maNhanVien, hoTenNhanVien, CCCD, ngaySinh, gioiTinh, diaChi, soDienThoai, chucVu, maChiNhanh, hinhAnh)
VALUES ('NVHN0001', N'Nguyễn Văn An', '123456289012', '1990-01-15', N'Nam', N'123 Đường Lê Lợi, Hà Nội', '0932345678', N'Nhân viên thu ngân', 'CNHN', ''),
	   ...
	   ('NVHCM0001', N'Lê Thị Hương', '123456712012', '1991-04-20', N'Nữ', N'123 Đường Lê Lai, Hồ Chí Minh', '09123325678', N'Nhân viên thu ngân', 'CNHCM', ''),
	   ...
	   ('NVCT0001', N'Nguyễn Thành Nhân', '123451489012', '1991-03-20', N'Nam', N'123 Đường Hòa Bình, Cần Thơ', '0912343278', N'Nhân viên thu ngân', 'CNCT', ''),
	   ...
	   ('NVDN0001', N'Trần Đinh Xu', '123453089012', '1991-02-15', N'Nam', N'123 Đường Bạch Đằng, Đà Nẵng', '0916845678', N'Nhân viên thu ngân', 'CNDN', ''),
	   ...
	   ('NVHUE0001', N'Lê Văn Hưng', '123456789012', '1991-05-20', N'Nam', N'123 Đường Nguyễn Huệ, Huế', '0332345678', N'Nhân viên thu ngân', 'CNHUE', ''),
...
VALUES ('PN0001', 'XE0001', '2023-10-01', 850000000 * 0.5, 5, 'NCC0001', 'CNHN'),
		('PN0002', 'XE0002', '2023-10-01', 1260000000 * 0.5, 5, 'NCC0002', 'CNHCM'),
		('PN0003', 'XE0003', '2023-10-01', 1180000000 * 0.5, 5,'NCC0003', 'CNHN'),
		('PN0004', 'XE0004', '2023-10-01', 1059000000 * 0.5, 5,'NCC0004', 'CNHUE'),
		('PN0005', 'XE0005', '2023-10-01', 1249000000 * 0.5, 5,'NCC0005', 'CNDN');
go
...
VALUES ('HD0001', '2023-10-01', 'XE0001', 850000000 * 1.1 + 20000000, N'Đã thanh toán', 'KH0001', 'NVHN0001'),
		('HD0002', '2023-10-01', 'XE0002', 1260000000 * 1.1 + 20000000, N'Đã thanh toán', 'KH0002', 'NVHN0002'),
		('HD0003', '2023-10-01', 'XE0003', 1180000000 * 1.1 + 20000000, N'Đã thanh toán', 'KH0003', 'NVHN0003'),
		('HD0004', '2023-10-01', 'XE0004', 1059000000 * 1.1 + 20000000, N'Đã thanh toán', 'KH0004', 'NVHN0004'),
		('HD0005', '2023-10-01', 'XE0005', 1249000000 * 1.1 + 20000000, N'Đã thanh toán', 'KH0005', 'NVHN0005');
go
...
BEGIN
	...
	SET @giaNhap = (select giaBan from XE where maXe = @maXePHIEUNHAP) * 0.5
    ...
    SET @startDatePhieuNhap = DATEADD(DAY, 3, @startDatePhieuNhap);
END;
--rollback
...
BEGIN
	...
	SET @tongTien = (select giaBan from XE where maXe = @maXe)
	SET @tongTien = @tongTien * 1.1 + 20000000
	...
	DECLARE @soLuong int = (select soLuongXeCon from KHOXE WHERE maXe = @maXe)
	IF (@soLuong > 0)
	begin
		INSERT INTO HOADON (ngayLapHoaDon, maXe, tongTien, tinhTrang, maKhachHang, maNhanVienThucHien)
		VALUES (@startDateHOADON, @maXe, @tongTien, N'Đã thanh toán', @maKhachHang, @maNhanVien)
		SET @startDateHOADON = DATEADD(DAY, 1, @startDateHOADON);
	end
END;
--rollback
...
--select XE.tenXe, sum(tongTien) as Total
--from XE inner join HOADON hd on XE.maXe = hd.maXe 
--where MONTH(ngayLapHoaDon) = '12' and YEAR(ngayLapHoaDon) = '2023'
--group by Xe.tenXe

--select XE.maXe, sum(tongTien) as Total
--from XE inner join HOADON hd on XE.maXe = hd.maXe 
--where MONTH(ngayLapHoaDon) = '12' and YEAR(ngayLapHoaDon) = '2023'
--group by Xe.maXe

--select distinct MONTH(ngayLapHoaDon) from HOADON where YEAR(ngayLapHoaDon) = '2024'
--rollback

--select sum(tongTien) from HOADON
--select sum(giaNhap * soLuong) from PHIEUNHAP


--select * from KHOXE
--select sum(soLuongXeCon) from KHOXE
