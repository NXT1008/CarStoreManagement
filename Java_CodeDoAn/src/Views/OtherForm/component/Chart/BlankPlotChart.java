...
//    }
    ...
    for (int i = 0; i <= niceScale.getMaxTicks(); i++) {
        String text;
        double scaledValue = valuesCount;
        if (valuesCount >= 1_000_000_000_000L) {
            text = format.format(scaledValue / 1_000_000_000_000L) + " Triệu Tỷ";
        } else if (valuesCount >= 1_000_000_000L) {
            text = format.format(scaledValue / 1_000_000_000L) + " Tỷ";
        } else if (valuesCount >= 1_000_000L) {
            text = format.format(scaledValue / 1_000_000L) + " Triệu";
        } else {
            text = format.format(valuesCount);
        }
        Rectangle2D r2 = ft.getStringBounds(text, g2);
        double stringY = r2.getCenterY() * -1;
        double y = getHeight() - locationY + stringY;
