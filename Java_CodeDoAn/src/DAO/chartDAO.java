...
public class chartDAO {
    ...
    public List<pieChartXeModel> listChartXe(String month, String year){
        List<pieChartXeModel> list = new ArrayList<>();
        String query = """
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()){
                list.add(new pieChartXeModel(rs.getString(1), rs.getLong(2)));
            }
            conn.close();
        } catch (Exception e) {
        }
    ...
    public List<String> listMonthByYear_HoaDon(String year){
        List<String> list = new ArrayList<>();
        String query = "select distinct MONTH(ngayLapHoaDon) from HOADON where YEAR(ngayLapHoaDon) = ?";
        try {
            conn = DBConnection.getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, year);
