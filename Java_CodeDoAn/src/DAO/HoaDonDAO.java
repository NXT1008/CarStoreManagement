...
public class HoaDonDAO {
    ...
    public List<HoaDon> danhSachHoaDonTheoNam(String year){
        List<HoaDon> list = new ArrayList<>();
        String query = "select * from HOADON where YEAR(ngayLapHoaDon) = ?";
        try {
            conn = DBConnection.getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, year);
    ...
    public long tongTienTheoNam(String year){
        long value;
        String query = "select sum(tongTien) from HOADON where YEAR(ngayLapHoaDon) = ?";
        try {
            conn = DBConnection.getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, year);
