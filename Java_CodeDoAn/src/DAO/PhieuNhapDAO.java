...
public class PhieuNhapDAO {
    ...
    public List<PhieuNhap> danhSachPhieuNhapTheoNam(String year){
        List<PhieuNhap> list = new ArrayList<>();
        String query = "select * from PHIEUNHAP where YEAR(ngayNhap) = ?";
        try {
            conn = DBConnection.getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, year);
    ...
    public long tongChiPhi(){
        long values;
        String query = "select sum(giaNhap * soLuong) from PHIEUNHAP";
        try {
            conn = DBConnection.getConnection();
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
    ...
    public long tongChiPhiTheoNam(String year){
        long values;
        String query = "select sum(giaNhap * soLuong) from PHIEUNHAP where YEAR(ngayNhap) = ?";
        try {
            conn = DBConnection.getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, year);
